...
**Ajouter une nouvelle ligne dans `archives.html`** [**RÉÉCRIT le 6 septembre 2026** — `archives.html` avait été refondu entre-temps en tableau (`<table class="archives-table" id="archives-table">`, filtres par domaine/scénario/impact France/révisé/lectures en JS) sans que cette routine en soit informée : l'ancien système `<ul class="entries" id="entries">` / `<li class="entry entry-weekly">` / fragment lazy-load au clic « Les 7 jours ▾ » décrit ici jusqu'au 5 septembre n'existait plus du tout dans la page réelle, et **les 5 récaps déjà publiés (09, 16, 23, 30 août, 6 septembre) n'apparaissaient nulle part dans `archives.html`** malgré des exécutions de cette routine qui pensaient les y avoir ajoutés à chaque fois — repéré et corrigé le 6 septembre (les 5 lignes ont été rétroactivement ajoutées ce jour-là). Nouveau format ci-dessous : une ligne `<tr class="row-weekly">` distincte, avec un lien direct vers `hebdo/{date}.html` — pas de fragment inline, pas de toggle. **Positionnement : toujours juste EN DESSOUS de la ligne `<tr data-date="{AAAA-MM-JJ du dimanche}">` de l'édition quotidienne de ce dimanche** (repérer son `</tr>` de fermeture, insérer juste après) :
```html
<tr class="row-weekly" data-date="{AAAA-MM-JJ du dimanche}-hebdo">
  <td class="col-date" data-label="Date"><span class="date-day">{JJ.MM}</span><span class="date-year">{AAAA}</span></td>
  <td class="col-title">
    <span class="title-row">
      <a href="hebdo/{AAAA-MM-JJ du dimanche}.html" title="Le récap de la semaine — les 7 sujets et leur scénario jugé le plus probable">On refait le scénario de la semaine — {date de début} au {date de fin}</a>
      <span class="weekly-badge">Récap hebdo</span>
    </span>
  </td>
  <td class="col-domain" data-label="Domaine"><span class="cell-na">—</span></td>
  <td class="col-eval" data-label="Notre scénario"><span class="eval-badge badge-na" title="Résumé de la semaine — voir le détail" aria-label="Résumé de la semaine — voir le détail">—</span></td>
  <td class="col-france" data-label="Impact France"><span class="cell-na">—</span></td>
...
```
**Pourquoi ce format plutôt que de forcer domaine/scénario/impact France** : un récap hebdo n'a pas un domaine ou un scénario uniques (il en résume 7), donc inventer une valeur serait faux. Les classes `row-weekly`/`weekly-badge`/`cell-na` (CSS déjà dans `archives.html`, ne pas les redéfinir) donnent un rendu discret et distinct (liseré doré à gauche, badge « Récap hebdo » à côté du titre) sans polluer les colonnes de données. **Ne pas mettre `data-domain`/`data-scenario`/`data-france`/`data-revised`** sur cette ligne : leur absence la fait disparaître automatiquement dès qu'un filtre autre que « Tous » est actif sur une de ces dimensions (comportement JS existant, rien à modifier) — c'est le comportement voulu. Le suffixe `-hebdo` sur `data-date` est nécessaire pour ne pas entrer en collision avec le `data-date` de la ligne de l'édition quotidienne du même jour dans `assets/data/reads.json` (le compteur de lectures). Ne jamais toucher aux filtres, à leur JS, ni à une ligne existante autre que celle ajoutée.
...
**Ne jamais toucher** à `archives/*.html`, `feed.xml`, `sujets-prioritaires.md`, `docs/sujets-a-suivre.md`, ni aucun autre fichier hors de ceux listés à l'étape 5. Sur `index.html`, seules modifications autorisées : les trois remplacements ciblés décrits à l'étape 4 (icône masthead + `data-hebdo` + `href` du bandeau `weekly-banner`) — jamais toucher `.intro-banner` ni son JS. Dans `archives.html`, ajouter uniquement la nouvelle `<tr class="row-weekly">` décrite à l'étape 4 — jamais modifier une ligne existante ni les filtres/JS. Ne jamais retoucher une page ou un fragment `hebdo/*` déjà publiés une semaine précédente.
