...
**Dans toutes les versions HTML de cette phrase d'ouverture** (le premier paragraphe de la `<description>` CDATA dans `feed-weekly.xml`, et le texte du bloc `.week-conclusion.week-conclusion-lead` sur `hebdo/{date}.html` et son fragment), **mettre en `<strong>` les 4-6 points vraiment importants** — dates clés, chiffres, noms d'institutions/organisations, la phrase de synthèse finale (« Le fil commun de la semaine : … »). Le CSS affiche déjà ce `<strong>` **en doré** (`.week-conclusion p strong{ color: var(--gold); font-weight: 600; }`, dans `hebdo/{date}.html` et `archives.html`) — ne pas ajouter de couleur inline dans le HTML, la couleur vient du CSS partagé. But : que le lecteur qui scanne rapidement capte l'essentiel sans tout lire, même principe que le gras déjà utilisé dans les paragraphes « pourquoi » des cartes de scénario sur `archives/*.html` (mais doré ici plutôt que blanc, pour se distinguer visuellement comme un vrai résumé). Ne pas gras-ifier des phrases entières ni sur-utiliser le procédé (4-6 passages en gras maximum, pas un mot sur deux) ; **exception : le `<comments>` du flux reste en texte brut sans aucun HTML** (donc sans `<strong>`), c'est la seule version de cette phrase qui n'en a pas.
