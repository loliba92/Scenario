...
**Objectif de fond (retour utilisateur du 19 septembre 2026), à garder comme boussole de la refonte** : l'enjeu n'est pas juste de vider une file plus vite pour vider une file — c'est de rester connecté à l'actualité réelle, montrer que Scénario est « dans le game », avec des sujets qui sont vraiment au cœur des préoccupations du moment. Un sujet qui attend plusieurs semaines dans le backlog avant publication rate cet objectif même s'il finit par sortir : la mesure de succès de la refonte n'est pas seulement « le stock ne grossit plus », c'est « ce qu'on publie reflète ce qui compte maintenant ».

### À faire
- Semaine du 22 septembre 2026 : repenser la logique de construction du backlog dans son ensemble (pas seulement `generate_hot_topics.py`) — comment les 3 canaux d'alimentation (utilisateur, routine quotidienne, workflow hebdo) et le seul canal de consommation (1 édition/jour) doivent s'articuler pour que le stock ne croisse pas indéfiniment, **en gardant l'objectif de fond ci-dessus** (pas juste un problème de débit à optimiser dans l'abstrait).
