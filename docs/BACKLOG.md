...
> Contenu déplacé depuis `docs/ARCHITECTURE.md` (sections « Backlog » et « Ce qui reste à faire (suivi) »), pour garder ARCHITECTURE.md focalisé sur l'architecture technique. Les renvois internes du type « voir plus haut/plus bas dans ce backlog » font désormais référence à ce fichier-ci.
...
- **P2 — Mettre davantage en avant les sujets suivis sur `archives.html`, idée du 29 août (retour utilisateur).** Constat : le `.suivi-badge` existant (voir plus haut, « Rendre visible le dernier sujet de suivi mis à jour ») fait déjà le travail fonctionnel (repérer et filtrer les sujets révisés), mais reste visuellement discret — un petit pill texte + emoji 🔄, au même niveau que le reste de la carte d'archive, qui ne saute pas vraiment aux yeux dans la liste. **Consigne explicite de l'utilisateur pour le calibrage : pas une grosse couleur, pas un énorme badge** — juste quelque chose d'assez visuel, porté par une icône, sans que ça devienne criard ou lourd dans la liste. Pistes à explorer au moment de traiter ce ticket (aucune tranchée) : une icône dédiée plus dessinée (SVG, cohérente avec le reste des icônes du site) plutôt que l'emoji 🔄 actuel ; un repère visuel sur la carte elle-même (ex. un liseré/pastille de coin) en plus du badge texte, pour qu'un sujet suivi se distingue au premier coup d'œil dans la liste sans avoir à lire le badge ; ou un simple traitement graphique plus marqué du badge existant (icône plus grande, meilleur contraste) sans toucher à sa taille de police ni à sa couleur dominante. Comparer plusieurs variantes visuelles avant de choisir, comme pour le format des cartes de scénarios le 28 août (voir plus haut/plus bas dans ce backlog) — ne pas se contenter d'un seul essai.
- **P3 — Mémoriser la langue de lecture choisie par le lecteur (FR/EN), idée du 30 août (retour utilisateur), validée P3 le même jour.** À ne pas confondre avec la détection automatique de langue navigateur, explicitement écartée le 29 août (`docs/strategie-anglais.md` § « UX de bascule entre langues » : « pas de détection de langue navigateur, pas de redirection automatique — le bouton offre un accès direct, jamais un choix imposé ») — cette règle vise un nouveau visiteur qu'on redirigerait de force sans qu'il ait rien demandé, le français doit rester la langue par défaut pour ça. Le ticket ici est différent : mémoriser un choix explicite déjà fait par le lecteur (clic sur le bouton `.masthead-lang-btn` EN/FR existant), pour le réappliquer aux visites suivantes — compatible avec la règle du 29 août, ne la remet pas en cause.
  ...
  - **Le vrai coût est le déploiement, pas la logique** : le site est statique (pas de template partagé côté serveur), donc ce script doit être ajouté sur toutes les pages — une passe rétroactive sur l'existant, puis une ligne de plus dans le gabarit recopié chaque jour par `docs/routine-prompt.md`/`docs/routine-en-prompt.md`.
  - **Répond au cas PWA soulevé par l'utilisateur** (« quand il ouvre l'appli, ça ouvre en anglais ou français ») : le site a deux manifestes distincts (`manifest.webmanifest` racine pour le FR, `en/manifest.webmanifest` pour l'EN — voir `docs/strategie-anglais.md`), donc l'icône ajoutée à l'écran d'accueil ouvre toujours la même langue de départ quelle que soit la préférence. Comme ce script tournerait après ce premier chargement, il rebondirait automatiquement vers la langue préférée à chaque ouverture, y compris pour une app installée depuis l'autre langue.
...
## Backlog produit / éditorial

## Audit externe du 27 août

Comparé point par point à l'état réel du dépôt avant d'agir, même méthode
que l'audit LLM du 20 août documenté plus bas dans ce même fichier. L'utilisateur a soumis un backlog de 23 tickets
...
- **Vote communautaire / affichage des résultats** : déjà en réflexion
  active côté technique — voir plus haut dans ce backlog l'entrée « Faire remonter le vote quotidien sur le site en plus de Telegram, idée du 26 août ». Mécanisme (clic →
...
- **Mesure de calibration** : déjà loggée plus haut dans ce backlog, entrée « Page de calibration ("avions-nous raison, au global"), idée du
...
- **Données ouvertes / API publique** : déjà loggée plus haut dans ce backlog, entrée « Données ouvertes / API publique, idée du 10 août ». `feed.json` existe déjà en JSON
...
documenté (plus haut dans ce backlog, entrée du 26 août) comme fonctionnant
...
correspondante : plus haut dans ce backlog, section « Distribution /
...
mise en page 2 pages A4 — « Les Cahiers de Scénario » — est versionnée
dans ce dépôt : `docs/mockups/cahier-scenario-2026-08-25.html` (ouvrir
tel quel dans un navigateur, imprimable A4 via Ctrl/Cmd+P). Aussi publiée
comme Artifact (peut avoir divergé du fichier versionné si retouchée
depuis côté Artifact sans re-synchro) :
