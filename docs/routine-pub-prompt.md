...
**3 septembre : le mécanisme photo de la catégorie `chiffre` (étape 2,
...
**3 septembre : catégorie `citation` mise en pause, remplacée par
...
`eyebrow`/`message`/`attribution`/`cta` (et `stat` pour la catégorie
`chiffre`) et ajouter l'item correspondant à `en/feed-pub.xml` (`<guid>` =
...
dans `en/assets/social/pub/{AAAA-MM-JJ}.png`. Procédure complète :
