...
- **Référence complète (CSS + SVG + script JS de génération) à copier telle quelle plutôt qu'à réinventer** : `index.html` / `archives/2026-08-21.html`, édition « Le Grand Filtre », bloc `.dc-chart-box` + script `dc-svg`. En l'adaptant : le tableau `data` (année/valeur en unité fine, ex. secondes plutôt que minutes arrondies), la couleur (`--degrade` si la série est alarmante, sinon la couleur de scénario la plus proche du sens de la série), les repères `isPeak`/`isLast` (le point extrême à mettre en avant, jamais plus d'un ou deux par graphique) et le sous-ensemble d'années affiché sur l'axe X (jamais les 30+ années en entier, ça surcharge). CSS pas encore dans le gabarit de base (contrairement à `.comprendre-box`) : à copier depuis l'édition de référence, pas à réécrire de zéro.
