...
vérité vivante, pas une copie** : le modifier ici (commit + push sur `main`)
suffit à changer le comportement de la routine dès sa prochaine exécution.

**Limite connue** : ce trigger a été créé via l'API HTTP, pas par un agent —
`update_trigger` y est refusé pour un agent. Modifier le **prompt-pointeur
lui-même** (sa mécanique, pas le contenu édité ici) reste donc un geste
manuel, mais rare : seulement si la mécanique du pointeur change, jamais
pour une règle éditoriale ou technique ordinaire.

**Fichier volontairement allégé** (coût token : chaque exécution repart d'un
conteneur neuf, aucun cache d'un jour à l'autre). Ne garde que les **règles
opérationnelles** — le récit de chaque correction (date, retour utilisateur
exact, exemple avant/après) vit dans `docs/ARCHITECTURE.md`, pas ici. Si une
règle ci-dessous semble mal calibrée en pratique, vérifier d'abord
`ARCHITECTURE.md` avant de la réinterpréter. (Historique des allègements
successifs : voir `docs/ARCHITECTURE.md` § « Automatisation éditoriale » —
un rollback vers une version datée d'avant plusieurs semaines de règles
ferait perdre ces règles, ne jamais l'utiliser comme raccourci.)
...
**Étape 0 — Sujet prioritaire (avant l'auto-sélection).** Lire `sujets-prioritaires.md` à la racine. S'il y a une ligne non cochée sous « 🔥 Priorité absolue », prendre la première → sujet du jour, quel que soit le registre. Sinon, dans la section du registre du jour, prendre la première ligne non cochée **qui n'est pas marquée 🔍** (voir juste en dessous). Si le sujet imposé tombe sous une restriction (fait divers violent, personne privée nommée, etc.), le laisser décoché et passer au suivant / à l'auto-sélection. Si rien ne correspond, auto-sélection normale.

**Ne plus cocher la case toi-même (changé le 19 septembre 2026).** Ancienne consigne : « après publication réussie, cocher la case ». Devenue fausse depuis le 14 septembre — tu ne publies plus toi-même, tu t'arrêtes après avoir écrit le brief et déclenché le pipeline, bien avant la publication réelle (voir STATUT ACTUEL en tête de fichier) ; « publication réussie » ne se produit jamais sous tes yeux. Résultat concret avant ce correctif : plus personne ne cochait rien, sur aucune édition (ex. Taïwan le 14, ARENH le 16, restées `- [ ]` alors que publiées). **À la place** : quand le sujet du jour vient de cette étape (pas de l'auto-sélection normale), reporte le texte exact de la ligne utilisée dans `sujet.origine_prioritaire` du brief (voir `docs/routine-brief-format.md` § dédié) — `generate_post_edition.py --publish` coche la case automatiquement une fois la publication réelle confirmée. `null` si le sujet vient de l'auto-sélection normale.

**Marqueur 🔍 — proposition automatique pas encore validée par l'utilisateur.** `scripts/edition/generate_hot_topics.py` (recherche DeepSeek, voir ce script) ajoute ses propositions avec un préfixe `🔍` devant le titre, précisément pour qu'elles ne soient jamais prises telles quelles par cette étape — l'intention d'origine (voir docstring du script) est qu'elles restent une simple suggestion en attente d'une revue humaine avant de compter comme un vrai sujet prioritaire. **Une ligne `🔍` ne doit jamais être choisie par l'auto-sélection, quel que soit son âge** : passer directement à la ligne suivante (ou à l'auto-sélection normale du registre si c'est la seule candidate). Elle ne redevient éligible qu'une fois le `🔍` retiré à la main (validation utilisateur) — jamais retiré automatiquement par une routine.
...
2. **Mêmes registre, 2-3 dernières occurrences.** Un registre ne revient qu'une fois par semaine (ex. jeudi économie mondiale) — un chevauchement à J-7 passe sous le radar du seul test « veille ». Avant de valider, relire dans `archives.html` les 2-3 dernières éditions **du même registre que celui du jour** (pas toutes les éditions, seulement celles du même créneau récurrent). Si le sujet candidat partage le même moteur de fond avec l'une d'elles — même événement déclencheur, même mécanisme causal central, même si l'angle éditorial diffère (ex. une édition traite l'inflation causée par un choc pétrolier, une autre la réaction des banques centrales au même choc) — l'écarter et passer au candidat suivant. Un chevauchement avec une édition plus ancienne que ces 2-3 dernières occurrences n'est pas bloquant.

Exemple : « Inflation : le pétrole rebat les cartes » et « Taux : marche arrière », deux jeudis économie à 7 jours d'écart, motivées par le même choc pétrolier (détroit d'Ormuz) — le test veille seul n'aurait rien vu.

### Étape 1 — Sélection automatique du sujet du jour
Registre imposé par le jour (heure de Paris) : Lundi géopolitique (conflits, diplomatie, rapports de force entre États) · Mardi Grand futur (tendances et ruptures à horizon long, 5 à 30 ans, tous domaines : technologie, démographie, ressources & énergie, exploration spatiale, IA, recherche…) · Mercredi actualité/politique française · Jeudi économie & finance mondiale (marchés, monnaies, dette, matières premières, entreprises) · Vendredi sciences au sens large (écologie, espace, IA, médecine, énergie, recherche) · Samedi culture (française et internationale, sans distinction) · Dimanche sport (enjeux sportifs/économiques, jamais la vie privée des sportifs).

**Mardi = « Grand futur »** (plus « libre »). L'ancien registre du mardi (« plus fort enjeu/incertitude tous domaines », en pratique une actualité chaude comme les autres jours) est remplacé par un registre prospectif : sujets à horizon long (5 à 30 ans) plutôt que de l'actualité immédiate — ruptures technologiques, démographie, ressources/énergie, exploration spatiale, IA, grandes tendances de recherche. **Toujours au conditionnel** pour ce qui n'est pas encore arrivé (« pourrait », « devrait » — jamais « sera », « va révolutionner »), même règle que la catégorie `futur` de `docs/routine-pub-prompt.md`. Jamais une généralité déjà connue de tous : un fait précis, daté, spécifique, avec une vraie incertitude sur son issue (les 3 scénarios doivent rester réellement ouverts, pas un simple constat que « la technologie progresse »).

**Ceci ne change rien à l'Étape 0 : la section « Mardi — carte blanche aux lecteurs » de `sujets-prioritaires.md` garde la priorité.** Si un lecteur a proposé un sujet (email à scenariocontact75@gmail.com, recopié dans cette section par l'utilisateur), il passe toujours avant le Grand futur — Grand futur n'intervient qu'en auto-sélection, quand cette section est vide ou entièrement cochée, exactement comme l'ancien registre « libre » avant lui. Un sujet suggéré par un lecteur reste traité normalement même s'il ne relève pas d'un horizon long (voir note dans `sujets-prioritaires.md`).

**Samedi et dimanche** : l'ancien duo « culture française / culture internationale » fusionne en un seul registre « culture » (samedi). **Règle de classement pour un sujet à cheval géopolitique/économie** (ex. guerre commerciale, tarifs douaniers) : l'enjeu central est un rapport de force entre États (qui menace qui, qui négocie quoi) → lundi géopolitique ; l'enjeu central est un indicateur chiffré ou un marché (prix, taux, dette, cours) → jeudi économie. Voir `sujets-prioritaires.md` (section « Géopolitique — lundi ») et `docs/tags.md` §1 pour les tags associés (`culture`, `economie-mondiale` ; `culture-francaise`/`culture-internationale` sont des tags historiques, ne plus les utiliser).

**Sport et Économie & finance mondiale, permutés** — Sport le dimanche, Économie & finance mondiale le jeudi (sujets plus légers le week-end).

**Économie & finance mondiale (jeudi) — un seul foyer géographique par édition, sauf si le sujet est justement la divergence entre deux zones.** Un sujet centré sur un indicateur mondial (taux, inflation, dette, marché…) doit choisir **une zone/institution comme fil conducteur** (ex. la Fed et l'économie américaine, ou la BCE et la zone euro) plutôt que de traiter systématiquement États-Unis et Europe à parts égales — mélanger les deux dilue l'article et le lecteur perd le fil de qui fait quoi et pourquoi. **Exception explicite : si la problématique du jour porte elle-même sur un écart, un désaccord ou une divergence entre deux zones** (ex. « pourquoi la BCE monte ses taux quand la Fed les gèle »), alors couvrir les deux camps est le sujet lui-même, pas une dilution — dans ce cas, toujours aller au bout et expliquer la cause réelle de l'écart (mécanismes structurels différents, pas juste juxtaposer deux décisions sans les relier), jamais se contenter de constater que les deux bougent différemment.
...
**Restrictions absolues**, même si le sujet correspond au registre : jamais un fait divers violent, jamais une personne privée nommée, jamais un sujet à caractère sexuel, jamais un sujet polémique sans enjeu factuel clair. **Jamais un sujet dont l'acteur central est un homme dans un rôle personnel/privé** (polémique de comportement, vie privée, réputation individuelle) **sauf s'il agit dans une fonction institutionnelle** (chef d'État ou de gouvernement, ministre, élu, chef religieux, magistrat, chef militaire, dirigeant d'une institution publique/internationale — président, pape, gouverneur et équivalents) — pour ne pas prêter le flanc à des rumeurs sur un homme qui n'a pas ce statut public. Un homme cité dans un rôle factuel/institutionnel n'est pas concerné par cette restriction (ex. un PDG dont l'actualité porte sur une décision économique vérifiable de son entreprise, pas sur sa personne). Si aucun sujet du registre strict ne convient, élargir au registre au sens large plutôt que forcer un sujet non pertinent.

**Sélection : conséquence réelle pour le lecteur français avant tout, jamais une actualité "chaude" par simple buzz/émoi.** Écarter un sujet dont l'intérêt tient surtout à l'agitation qu'il suscite (réseaux sociaux, polémique de personnalité, célébrité) sans conséquence structurelle vérifiable (économique, politique, scientifique, sociétale) — même règle que la restriction ci-dessus sur les rumeurs, dans l'autre sens : un sujet sans enjeu factuel réel n'est pas un bon sujet, même s'il "buzz". Scénario est un site français, lu par des Français : à candidats comparables, préférer celui qui a une vraie portée ou un lien concret pour un lecteur français (pas besoin d'être un sujet franco-français — un sujet mondial avec un enjeu réel convient très bien — mais éviter un sujet purement anecdotique à l'étranger sans aucune résonance ni conséquence côté France).
...
**Le h1 doit nommer le pays/la zone concernée dès qu'elle n'est pas la France** (lectorat français par défaut). Un lectorat français lit un h1 générique (« Dette : le mur des 40 000 milliards », « Taux : marche arrière ») en supposant par défaut qu'il s'agit de la France, sauf mention contraire — l'ambiguïté n'est levée que plus bas, dans la question posée ou les `.dek`, que tout le monde ne lit pas. Dès que le sujet du jour porte sur un autre pays/zone (États-Unis, Chine, UE hors France, un conflit étranger...), l'écrire explicitement dans le h1 lui-même (ex. « Dette américaine : le mur des 40 000 milliards », pas « Dette : le mur des 40 000 milliards »), même si ça ajoute un ou deux mots — jamais au prix de la longueur/du punch au point de rendre le h1 lourd, mais jamais sacrifié non plus pour rester court. Aucune mention nécessaire seulement si le sujet est explicitement français (registre du mercredi) ou mondial/sans foyer unique (ex. un sujet scientifique global) — le doute profite toujours à la précision, pas à la brièveté. Cas réel : une édition entièrement sur la dette et le Trésor américains avait un h1 disant juste « Dette », lu par défaut comme la dette française.
...
**Cas plus exigeant : une donnée cotée en continu** (prix de marché, taux
de change, indice boursier) n'est jamais un instantané qu'on fige une
fois pour toutes à la recherche — elle continue de bouger jusqu'à la
publication effective. Voir la règle dédiée en Phase 2 (§ Anti-péremption
plus bas) pour la revérification à faire juste avant mise en prod, pas
seulement à cette étape de recherche.
...
**Décisions sur les encarts** (`encarts_decides`) : d'abord noter
`sujet.complexite` (entier 1 à 5, voir `docs/routine-brief-format.md`) —
**au moins 1 `comprendre_box` toujours, exactement 2 à partir de
complexite ≥ 3** (changement du 16 septembre 2026, ajusté le même jour
en après-midi — voir plus bas dans ce fichier, § Encart « Comprendre »,
toujours valable comme grille de décision même si tu n'écris plus le
texte de l'encart) et sur quel focus chacun — deux mécanismes
**réellement distincts** du sujet, jamais deux angles du même ; un
`list_box` pertinent existe-t-il (matrice/classement/chronologie de
jalons, jamais un simple paragraphe déguisé) ? De même pour
`graphique_dc_chart` : § « Graphique en escalier » plus bas reste la
grille de décision valable (3 critères, jamais forcé) — mais renseigne
`serie` en JSON déclaratif (voir `docs/routine-brief-format.md`), plus un
script JS à écrire à la main, `build_html.py` s'occupe du rendu depuis le
15 septembre 2026.
...
1. Ouvrir `sources-log.json` (racine du dépôt) — un objet par jour dans `"days"`. Ajouter un nouvel objet (ou compléter celui du jour s'il existe déjà) : `{"date": "AAAA-MM-JJ", "articles": [{"title", "source", "url", "image", "lang" ("fr"/"en"/"other"), "domain" (un des 6 slugs de `docs/tags.md`), "summary", "read_minutes"}, ...]}`. `read_minutes` : estimation simple, pas la peine de chronométrer.
   **`image`** (une vignette par article, pas juste du texte) : l'URL de l'image principale (`og:image`) de l'article source — jamais téléchargée ni hébergée chez nous, un simple hotlink vers l'image telle qu'elle vit chez le média d'origine. Choix assumé, pas un oubli : recharger l'article, chercher la balise `<meta property="og:image" ...>` dans son HTML (`curl` suffit, pas la peine d'un navigateur complet), copier l'URL telle quelle. Laisser vide si l'article n'a visiblement pas d'image exploitable (og:image absent) — jamais une image générique ou hors-sujet à la place. **Limite connue et acceptée** : un hotlink dépend du média (URL qui peut changer, hotlink parfois bloqué par leur serveur) — `scripts/seo/generate_sources_page.py` masque silencieusement la vignette si l'image ne charge plus (`onerror`), jamais de lien cassé visible. Le média n'est jamais crédité par son nom sous l'image (déjà visible juste à côté, `.sources-meta`) — inutile de dupliquer.
2. Lancer `python3 scripts/seo/generate_sources_page.py` — régénère `sources.html` à partir de ce fichier, aucune édition manuelle de `sources.html` lui-même. Économie de tokens : cette étape est déterministe (lecture JSON + script), pas de jugement éditorial au-delà du choix des 2-5 liens et de leur résumé — la routine n'a jamais à relire toute la page ni tout l'historique du JSON pour ajouter le jour courant.
3. Committer `sources-log.json` et `sources.html` avec le reste de l'édition du jour, même commit.
...
**Donnée cotée en continu citée comme « chiffre du jour » (prix de marché, taux de change, indice, décompte de victimes en conflit actif) : revérifier sa valeur la plus récente à ce stade de la rédaction, jamais seulement reprendre celle trouvée en Phase 1.** Root cause d'un incident réel (édition du 22 septembre 2026, crise Bab el-Mandeb/Ormuz) : le brief avait été écrit la veille avec un prix du Brent déjà vieux de plusieurs jours au moment de la rédaction, et ce prix n'a plus bougé jusqu'à la publication — alors que le marché, lui, avait continué d'évoluer (repli de plusieurs dollars entre-temps sur fond de signaux diplomatiques, non mentionné). Entre la Phase 1 (brief, généré à 14h UTC la veille) et la mise en prod effective (le lendemain matin, voir `docs/PREVIEW-WORKFLOW.md`), suffisamment de temps passe pour qu'un cours de marché ait bougé. Avant de considérer la rédaction terminée : relancer une recherche datée du jour sur chaque donnée cotée en continu utilisée comme chiffre structurant, et si elle a bougé depuis le brief, l'intégrer dans le texte (pic + valeur la plus récente, comme une trajectoire plutôt qu'un chiffre isolé) plutôt que de laisser une valeur périmée.
...
**Toute affirmation attribuée nommément à une entité identifiée (banque, agence, institut, étude) doit correspondre à une source réellement consultée, listée dans `sources`.** Ne jamais écrire « selon [nom d'une banque/agence], [chiffre] » sans avoir retrouvé cette déclaration ou cette estimation précise chez cette entité — un chiffre plausible prêté à une source qui ne l'a jamais dit est pire qu'un chiffre non attribué, car il porte une autorité empruntée et invérifiable pour le lecteur. Si la recherche ne retrouve pas l'attribution exacte : soit chercher la vraie source qui porte ce chiffre (souvent une institution différente, ex. une agence officielle plutôt qu'une banque privée), soit reformuler sans le nom propre (« selon les estimations disponibles » reste honnête, un nom d'institution non vérifié ne l'est pas).
...
**Jamais de rupture de registre oral/conversationnel dans un `.dek`.** Le `.dek` garde un ton analytique à la troisième personne d'un bout à l'autre — le tutoiement et l'adresse directe au lecteur restent réservés aux éléments d'interface déjà dans le gabarit (nav, CTA, `.retenir-box`, etc.), jamais glissés au milieu d'un paragraphe d'analyse. Exemple réel à ne pas reproduire (édition du 22 septembre 2026) : « on avait déjà vu passer un sujet similaire sur la facture énergétique, n'hésite pas à lire notre article » — un aparté oral qui casse le ton au milieu d'un paragraphe sur les marchés de l'énergie. Un renvoi vers une édition passée reste possible et bienvenu (voir plus haut, recoupements), mais s'intègre comme une clause factuelle de la même phrase analytique, jamais comme une adresse directe au lecteur.

**Ouvrir sur un fait très récent quand il y en a un — à l'essai, jamais obligatoire.** Avant de rédiger le premier `.dek`, vérifier si un fait daté d'un jour ou deux (une visite en cours, une annonce, un mouvement de marché, une déclaration...) touche directement le sujet du jour. Si oui, envisager de l'utiliser comme point d'entrée du tout premier paragraphe, plutôt que de commencer par un rappel plus ancien du dossier — ça montre au lecteur, dès la première phrase, que le sujet est chaud aujourd'hui et pas juste ressorti d'un tiroir. Exemple qui a inspiré cette règle (édition du 7 septembre, Groenland) : ouverture sur la visite de von der Leyen « les 6 et 7 septembre », deux jours avant la publication, plutôt que sur le contexte plus ancien (menaces de Trump depuis janvier 2025) — qui garde toute sa place, juste un cran plus loin dans le récit. **Idée en observation, pas encore une règle éprouvée** : à tester édition après édition, jamais au prix de forcer une actualité artificiellement datée ou de sacrifier la clarté chronologique du récit qui suit. Si aucun fait vraiment récent n'existe pour le sujet du jour, ouvrir normalement sur le fait le plus structurant, comme avant.
...
**Profondeur obligatoire : ne jamais s'arrêter à la première explication qui suffit.** Une édition qui couvre les faits sans creuser reste correcte mais pauvre — le lecteur doit repartir avec plus que ce qu'un simple résumé de dépêches lui aurait donné. Avant de considérer le contexte terminé, se poser systématiquement ces trois questions et intégrer la réponse quand elle apporte une vraie clé de lecture (jamais en forçant si le sujet ne s'y prête pas — même logique que `.comprendre-box`/`.list-box`, voir plus bas) :
- **Un chiffre structurant est-il cité en toutes lettres, avec une source datée, ou seulement décrit qualitativement ?** (« pilier économique », « acteur majeur » ne remplacent jamais un vrai pourcentage vérifié — voir Anti-péremption ci-dessus. Exemple : « pêche islandaise, pilier économique » sans chiffre vs. « 21 % des exportations en 2024, en baisse depuis 38,8 % en 2021 » — un chiffre daté et sourcé change la lecture, une formule vague non.)
...
**Seuil quantitatif de longueur, complément mécanique du test des 3 questions ci-dessus.** Un article peut cocher formellement les 3 questions ci-dessus tout en restant nettement plus court que d'habitude — le test qualitatif seul n'a pas suffi une première fois (édition à 866 mots jugée trop pauvre malgré des réponses formellement correctes). Avant de considérer le contexte terminé (donc avant l'étape technique de publication) : compter les mots de `.dek`+`.why`+`dd` (même méthode que le calcul du temps de lecture, étape technique 8) et viser un minimum de **1100 mots** — calibré sur les éditions publiées fin août/début septembre 2026 (1088 à 1442 mots selon les sujets). **Sous ce seuil, rouvrir le test des 3 questions en profondeur** plutôt qu'en surface : laquelle des trois n'a en fait reçu qu'une réponse partielle ou qualitative ? Ajouter le contenu manquant (chiffre sourcé, second `.comprendre-box` si un vrai second angle existe, précédent comparable) plutôt que délayer une phrase existante — jamais de remplissage stylistique pour gonfler le compte de mots. **Un sujet réellement creusé qui reste sous le seuil peut être publié tel quel** : ce seuil attrape un manque de profondeur, pas une contrainte arbitraire — un sujet qui n'a vraiment rien de plus à offrir une fois les sources épuisées ne doit jamais être gonflé artificiellement. **Filet de sécurité redondant** : l'Inspecteur (`docs/routine-inspection-prompt.md`, point 12) revérifie ce même seuil une heure plus tard et enrichit lui-même si besoin — mais ce filet doit rester un rattrapage rare, jamais le mécanisme principal ; le respecter dès la rédaction évite la plupart des déclenchements.

**Liste à puces dans le contexte : obligatoire, une par édition.** Jusque-là simple exception au « pas de liste » ci-dessus, réservée aux vraies matrices de faits parallèles (plusieurs entités face à plusieurs acteurs ou options, avec pour chaque combinaison un statut discret et comparable — exemple : 3 maisons de disques × 2 plateformes IA, soit 6 duos, chacun accord signé ou procès en cours). Ce composant devient un passage obligé de chaque édition, au même titre que « Ce qu'on évalue » ou « L'essentiel » — mais le critère de fond ne disparaît pas pour autant : toujours un vrai ensemble organisé et comparable, jamais 2-3 faits qui s'enchaînent segmentés en puces pour la forme. Comme un sujet du jour n'offre pas toujours une vraie matrice N × M, construire la liste, dans cet ordre de préférence :
...
- CSS : classe `.list-box` (encart détaché du texte — fond `--surface`, bordure dorée, label JetBrains Mono, repère `.list-box-rank` **toujours en numéro, jamais en emoji**, voir règle emoji plus bas) — voir `docs/ARCHITECTURE.md` § « Encart liste » pour la structure HTML complète, et `archives/2026-08-08.html` pour un exemple en édition réelle. Remplace l'ancienne classe `.dek-list` (simples puces à tiret doré, sans encadré) pour toute nouvelle liste — `.dek-list` reste présent dans quelques éditions passées mais ne doit plus être utilisé pour du nouveau contenu. Ne pas en redéfinir une variante par édition ; si absente du `<style>` d'`index.html` pour une raison quelconque, la recopier telle quelle depuis l'édition de la veille (section CSS juste après `.essentiel-box`) plutôt que d'improviser une variante.

**Encart « Comprendre » — au moins un par édition depuis le 16 septembre 2026 (retour utilisateur : « plus pédagogique »), deux si le sujet est complexe.** Composant `.comprendre-box`, distinct du lexique et des `.dek` : sert à donner au lecteur **une** clé de lecture qui change sa manière de voir le sujet — un mécanisme, une distinction ou une analogie qui recontextualise l'enjeu, jamais une définition de terme (→ lexique) ni une reformulation de ce qui est déjà dit dans les `.dek`. Objectif explicite : l'effet « ah, je comprends mieux », pas un résumé de plus.

- **Choisir le focus en rédigeant le contexte, pas après coup.** Se demander « quelle est LA notion qui, une fois comprise, change la lecture du sujet ? ». Bon candidat : une distinction économique/structurelle contre-intuitive, un mécanisme caché, un biais de raisonnement répandu (ex. distinguer investissement productif et dépense défensive, pour comprendre pourquoi un budget climat ne « rapporte » jamais comme un investissement classique). Mauvais candidat : un sigle ou un terme technique isolé (→ `.lex-ref` + lexique, jamais ce composant).
- **Toujours rattacher explicitement le focus à la question du jour, jamais un aparté qui plane à côté du sujet.** Nommer dans le texte les acteurs/enjeux déjà installés par les `.dek` qui l'entourent (ex. sur une édition « banques centrales/taux » : dire explicitement que ce sont les décisions de politique monétaire qui sont en jeu, pas seulement parler de l'indicateur en vase clos) — l'encart doit se lire comme un zoom sur un fil déjà tissé, jamais comme une parenthèse autonome qu'on pourrait déplacer dans une autre édition sans rien changer.
- **Le nombre suit `sujet.complexite` (1 à 5, voir `docs/routine-brief-format.md`), jamais un choix libre.** En dessous de 3/5 : **au moins un** `.comprendre-box`, jamais 0. À partir de 3/5 : **exactement deux**, jamais moins, jamais plus. Ajusté le 16 septembre 2026 en après-midi (la version du matin forçait toujours deux, quel que soit le sujet — revu par retour utilisateur : forcer deux mécanismes distincts sur un sujet déjà simple pousse justement vers le contenu générique décrit plus bas). Ni l'un ni l'autre cas n'autorise à fabriquer un focus faible pour atteindre le compte : chercher activement, dans les faits vérifiés et le contexte, un (ou deux) mécanismes réellement distincts qui méritent d'être éclairés — presque tout sujet a un angle chiffré/structurel ET un angle acteurs/dynamique de pouvoir, ou un mécanisme court terme ET un mécanisme long terme ; creuser jusqu'à les trouver plutôt que de se limiter au premier venu. **Ce qui reste absolument interdit** (le vrai risque d'un contenu générique/creux, à éviter à tout prix) : un deuxième encart qui reformule le premier sous un angle à peine différent, une analogie plaquée qui ne s'appuie sur aucun fait vérifié du brief, ou une phrase vague sans le chiffre/l'acteur concret qui l'ancre dans le sujet du jour. Test avant de valider un focus : peut-on citer le fait/chiffre précis du brief sur lequel il s'appuie, et dirait-on que ce focus est spécifique à CE sujet plutôt qu'à n'importe quel autre du même registre ? Si non aux deux, en chercher un autre — jamais le garder par défaut pour atteindre le compte requis. S'il y en a deux, toujours au moins un `.dek` entre les deux encarts, jamais adjacents.
...
- **Placement : dans le fil des `.dek`, jamais en fin de bloc.** Insérer juste après le paragraphe `.dek` qui introduit le fait qui justifie l'analogie — jamais avant le premier `.dek` (le lecteur a besoin du fait avant la clé de lecture), et jamais relégué juste avant `indicator-strip` ou le titre des scénarios : plaqué en fin de section, ça se lit comme un ajout secondaire plutôt qu'une explication qui éclaire le texte qu'on vient de lire.
- CSS `.comprendre-box`/`.comprendre-label`/`.comprendre-lead`/`.comprendre-text` déjà dans le gabarit (même recette que `.question-box` : fond `--surface`, filet doré 3px, radius 4px) — voir `docs/ARCHITECTURE.md` § « Encart Comprendre » pour l'historique, et étape technique 2 pour la règle de recopie intégrale du `<style>` (classe utilisée seulement certains jours).

**Graphique en escalier pour série historique longue — optionnel, jamais forcé, réutilisable, jamais un one-shot.** Composant `.dc-chart-box` (nom hérité de sa première utilisation, l'Horloge de l'Apocalypse — garder ce nom de classe même pour un autre sujet, la peine de le renommer par édition n'en vaut pas le risque de régression). Affiche une série numérique réelle et longue en complément d'un chiffre déjà cité dans `.indicator-strip` — jamais pour le remplacer, seulement pour lui donner une perspective historique. **Rendu automatiquement par `scripts/edition/build_html.py` depuis le 15 septembre 2026** (SVG généré côté serveur à partir de `graphique_dc_chart.serie`, voir `docs/routine-brief-format.md` pour le schéma exact) — les critères de décision et les règles ci-dessous restent la grille de jugement éditorial, seule la mécanique de rendu a changé (plus de script JS à écrire à la main).

- **Critère de déclenchement, les trois nécessaires en même temps** : (1) une source publique fiable documente l'historique complet, pas seulement le dernier chiffre ; (2) au moins 5 points réels, idéalement sur une période longue par rapport à l'âge du sujet — pas forcément une décennie si le sujet lui-même est plus récent (ex. une guerre en cours depuis 4-5 ans) ; (3) la série éclaire directement la question du jour, pas une curiosité à côté du sujet. Si un seul manque, pas de graphique — même logique que `.comprendre-box`/`.list-box` : la plupart des éditions n'en auront aucun, ne jamais en fabriquer un pour remplir un slot.
- **Systématiser l'évaluation, pas seulement quand un sujet s'y prête visiblement.** À chaque édition, une fois les 2 KPI de `.indicator-strip` choisis (étape technique, cohérence des KPI), évaluer lequel des deux (jamais les deux à la fois — un seul graphique par édition, comme pour `.list-box`/`.comprendre-box`) a le plus de chances de satisfaire les 3 critères ci-dessus, puis vérifier réellement via WebSearch/WebFetch avant de trancher. Si aucun des deux ne passe les 3 critères, ne pas insister — la plupart des éditions n'auront pas de graphique, c'est attendu, pas un échec.
- **Un sujet récent n'est pas un indicateur sans historique — ne pas confondre les deux.** Un événement d'actualité récent (crise, choc, escalade) porte très souvent sur une grandeur qui, elle, a une longue histoire publique et documentée : prix d'une matière première, taux de change, indice boursier, taux directeur, inflation, trafic maritime/aérien officiel, victimes cumulées d'un conflit, etc. « le sujet est en cours »/« sujet émergent »/« temps réel » n'est jamais à soi seul une raison valable de conclure « non » — c'est l'événement qui est récent, pas forcément l'indicateur. Pour un KPI de ce type (prix de marché, indice, taux, cumul officiel), partir du principe inverse de la prudence par défaut : présumer qu'un historique existe et le chercher activement avant d'écrire « non », plutôt que constater l'absence du chiffre du jour dans le brief et s'arrêter là.
- **`raison` doit prouver la recherche, pas seulement affirmer une conclusion.** Interdit d'écrire une justification générique du type « pas de série historique disponible » sans détail. `raison` doit citer ce qui a été effectivement vérifié : la requête ou la source consultée (ex. EIA, Investing.com, banque centrale, ONU...), la période couverte et le nombre de points réels trouvés, et lequel des 3 critères échoue précisément. Une `raison` qui ne permettrait pas de rejouer la recherche a posteriori est insuffisante.
...
- **Renseigner `graphique_dc_chart.serie` en JSON, voir `docs/routine-brief-format.md` pour le schéma exact et un exemple** (`aria_label`/`lead`/`caption`/`y_max`/`y_gridlines`/`x_axis_years`/`points`) — plus besoin d'écrire un script JS ni de copier un fichier de référence. `points` : année/valeur dans une unité fine (ex. secondes plutôt que minutes arrondies). `y_gridlines`/`x_axis_years` : un sous-ensemble choisi à la main, jamais tous les points/toutes les années si la série est longue (surcharge visuelle). `peak`/`peak_label` sur au plus un point marquant hors du dernier (ex. un record) ; `last`/`last_label` toujours sur le dernier point chronologique. Références historiques pour se donner une idée du rendu visuel final (avant l'automatisation du rendu) : `archives/2026-08-21.html`, édition « Le Grand Filtre » (Horloge de l'Apocalypse), et `archives/2026-08-24.html`, édition « Ukraine : ni paix ni victoire » (série avec un palier à cheval sur une année charnière). **CSS déjà dans le gabarit de base** (voir étape technique 2 pour la règle de recopie intégrale) — rien à y toucher, le rendu du contenu est maintenant entièrement piloté par les données.
  ...
  <div class="wrap">
    ...
    <div class="stakes-box">
      ...
      <ul class="stakes-branches">
        <li data-kind="favorable"><span class="stakes-tag">Favorable</span>{ce que ce scénario veut dire, en une phrase déclarative courte}</li>
        <li data-kind="stable"><span class="stakes-tag">Stable</span>{idem, scénario stable}</li>
        <li data-kind="degrade"><span class="stakes-tag">Dégradé</span>{idem, scénario dégradé}</li>
      </ul>
    </div>
    ...
    <div class="cards">
      <!-- les 3 cartes -->
```
**Format en liste étiquetée, pas une question ouverte.** Une ancienne version enchaînait les 3 branches dans un seul `<p class="stakes-text">`, en questions (« Est-ce que... ? ») — le lien entre chaque sous-question et son scénario n'était porté que par l'ordre de lecture, invisible pour qui ne connaît pas déjà la convention. Désormais une liste, une ligne par branche, chacune ouverte par un `<span class="stakes-tag">` (le mot-repère Favorable/Stable/Dégradé, coloré comme `.kind-tag` sur les cartes) : le rapprochement entre une branche et sa carte se fait d'un coup d'œil, avant même d'atteindre les cartes. Le texte de chaque branche passe aussi en phrase déclarative simple (« L'Anses résiste et refuse... », pas « Est-ce que l'Anses résiste et refuse... ? ») — le mot-repère qui ouvre la ligne dit déjà qu'il s'agit d'une branche, la forme interrogative n'ajoutait rien. CSS (`.stakes-box`, `.stakes-label`, `.stakes-branches`, `.stakes-tag`) déjà dans le gabarit ; `.stakes-text` reste défini en CSS pour compatibilité avec les archives déjà publiées, mais ne doit plus être utilisé dans une nouvelle édition.

**Les 3 branches doivent rester dans l'ordre favorable/stable/dégradé, une par scénario, jamais une question ouverte vague.** Avant de publier, vérifier explicitement que chaque scénario (titre + `why`) répond sans ambiguïté à sa branche — si un scénario semble à côté, corriger le scénario ou la branche. **Ce texte n'est plus repris dans `feed.xml`** (retiré de la Description le 28 août, voir étape technique 8 plus bas).
...
**Rappel « Reste informé » compact, fusionnant notifications + newsletter dans un seul bloc à 2 actions** [structure générale, à reproduire tel quel chaque jour, même statut que la ligne `.masthead-right` décrite plus bas] — le seul bouton d'activation des notifications vivait tout en bas de page, jamais vu par la plupart des lecteurs. **Telegram volontairement exclu** de ce bloc (le mécanisme de vote ne fonctionne pas actuellement, sujets trop complexes pour ce format — le lien reste seulement dans le bloc `.share-block` du bas). Point d'entrée juste après `</div>` qui ferme `.essentiel-box` (toujours à l'intérieur de `section.scenarios`, avant la fermeture de `div.wrap`/`section`) :
```html
...
**Ne pas confondre avec « Ce qu'on évalue » (`.stakes-branches`, ex-`.stakes-text`) : `<source>` est réservé à « L'essentiel » et rien d'autre.** Erreur déjà commise une fois : « Ce qu'on évalue » copié dans `<source>` à la place de « L'essentiel » — vérifier que les deux textes ne sont jamais permutés. `<source>` alimente directement `{{4.source.title}}` sur les légendes Instagram/LinkedIn/Facebook (voir `docs/ARCHITECTURE.md`) : une confusion ici se propage silencieusement à tous ces canaux. **« Ce qu'on évalue » ne va nulle part dans `feed.xml`**, uniquement sur le site.
...
**Repère visuel favorable/défavorable sur `.france-line`** (pro et discret, mais voyant rapidement). Deux éléments, tous deux pilotés automatiquement par `data-france-impact` (aucune classe CSS supplémentaire à choisir à la main) :
...
- **« Ce qu'on évalue » (`.stakes-branches`, format complet plus haut dans l'étape 3) : chaque branche est une phrase déclarative courte**, jamais une phrase à rallonge avec points-virgules et incises. Mauvais : « L'Anses refuse la dérogation, sous X, par Z ; ou l'accorde, avec Y ; ou l'étend — D — jusqu'à E. » Bon, une ligne par branche : « L'Anses refuse, ou limite fortement, la dérogation. » / « Elle l'accorde exactement comme prévu par la loi. » / « La dérogation s'étend au-delà du périmètre initial. » (Format en liste étiquetée — voir Étape 3 — plutôt que l'ancien format en 3 phrases interrogatives dans un unique `<p>`.)
...
**Éviter les tournures qui sonnent artificielles/« IA ».** Deux pièges fréquents :
...
**Français naturel partout, pas seulement sur ces deux pièges.** Le test de la lecture à voix haute ci-dessus s'applique à toute phrase de l'édition, pas seulement aux encarts Comprendre — et pas seulement aux deux cas déjà listés. Symptômes fréquents à repérer et corriger avant publication :
...
2. Lire `index.html` actuel : gabarit de design exact à reproduire. Ne jamais changer le CSS ni la structure HTML générale — seulement le contenu texte et les valeurs. **Recopier le bloc `<style>` en entier, classe par classe, sans jamais filtrer sur ce qui sert au contenu du jour** — plusieurs classes (`.list-box`, `.comprendre-box`, `.dc-chart-box`, `.intro-banner*`, `.follow-inline*`, `.masthead-right`/`.masthead-notif-btn`, la règle 3 colonnes de `.cards`...) ne sont utilisées que certains jours, mais doivent rester dans le CSS tous les jours. Un jour sans liste avait fait disparaître `.dek-list` du `<style>` (recopié uniquement ce qui semblait utilisé ce jour-là) : si un diff du `<style>` entre l'ancien et le nouveau `index.html` montre une classe en moins, c'est un signal d'alerte, jamais une simplification bienvenue. **L'ancienne bande `.top-updates` (boutons "Sujet révisé" / "Récap de la semaine" en pleine largeur juste sous la nav) a été supprimée le 29 août — ne plus jamais la reproduire.** Retour utilisateur : ces deux liens ne méritaient pas leur propre bande, ils ont rejoint la ligne `.masthead-right` (voir juste en dessous).

   **Ne jamais ajouter de menu déroulant "Archives"** (bouton `.topnav-archives` + panneau à 7 liens) — projet abandonné, brièvement réappliqué par erreur le 1ᵉʳ septembre puis retiré le jour même. Le lien `Archives` de `nav.topnav` reste un simple `<a href="archives.html">Archives</a>` (icône + texte, comme les autres liens de cette nav). Si l'envie d'un point d'entrée vers les pages thématiques revient, proposer une nouvelle piste plutôt que de réutiliser ce patron.

   **Ligne `.masthead-right`, à droite du `.brand` logo [structure générale à reproduire telle quelle]** — regroupe, dans l'ordre, la cloche notifications (ajoutée le 21 août, 3e point d'entrée vers l'abonnement OneSignal, avec le rappel compact après "L'essentiel" et le bloc dédié en bas de page), le bouton impression, un séparateur vertical `.masthead-divider` (purement visuel, `aria-hidden`, ajouté le 29 août), les liens "Sujet révisé", "Récap de la semaine" (ex-`.top-updates`, voir ci-dessus) et "Revue de presse" (ajouté le 4 septembre, voir juste en dessous), un second `.masthead-divider`, puis le lien "Tableau de bord" (ajouté le 3 septembre) — tous en icône seule, cercle 32px bordure or (`.masthead-notif-btn`), pour que toute la ligne tienne sur une seule ligne y compris en mobile :
   ```html
   <div class="masthead-right">
     ...
     <a class="masthead-notif-btn" href="sources.html" aria-label="Revue de presse" title="Revue de presse"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3.5" y="5.5" width="13" height="14" rx="1.5"/><path d="M16.5 8.5h2.5a1.5 1.5 0 0 1 1.5 1.5v9a1.5 1.5 0 0 1-1.5 1.5h-13"/><line x1="7" y1="9.5" x2="13" y2="9.5"/><line x1="7" y1="13" x2="13" y2="13"/><line x1="7" y1="16.5" x2="11" y2="16.5"/></svg></a>
     <span class="masthead-divider" aria-hidden="true"></span>
     <a class="masthead-notif-btn" href="dashboard.html" aria-label="Accès privé" title="Accès privé"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg></a>
   </div>
   ```
   **Tous en icône seule, jamais de texte ni de `.btn-label`** — le retour d'état de l'abonnement notifications passe par l'attribut `title` (voir `onesignalSetLabel`) ; les liens "Sujet révisé"/"Récap de la semaine"/"Revue de presse" gardent leur texte uniquement en `title`/`aria-label` (accessibilité + tooltip au survol). Le lien "Sujet révisé" pointe toujours vers `archives.html?tag=revise` (filtre générique, toujours à jour tout seul — ne jamais le changer). Le lien "Récap de la semaine" pointe vers le dernier `hebdo/{date}.html` publié : sa mise à jour est un geste séparé, pas une tâche de la routine quotidienne — recopier tel quel, ne jamais le recalculer ni le deviner. **Le lien "Revue de presse" pointe toujours vers `sources.html`** (page unique qui grandit d'un jour à l'autre, jamais un fichier par jour — voir étape technique dédiée plus bas pour l'alimenter) : recopier tel quel, jamais de date dans l'URL. CSS `.masthead-right`/`.masthead-notif-btn`/`.masthead-divider` déjà dans le gabarit (voir étape technique 2 pour la règle de recopie intégrale).

   **Lien "Accès privé" vers le dashboard** (icône cadenas, `aria-label`/`title` = « Accès privé » — jamais une icône de statistiques, déjà essayée et jugée ambiguë). Pointe vers `dashboard.html` (page interne protégée par un code d'accès — voir `docs/routine-audience-prompt.md` étape 3bis et `docs/BACKLOG.md`, entrée « Dashboard de pilotage interne »). `cursor: pointer` déjà hérité de `.masthead-notif-btn`, rien à ajouter côté CSS. **Ce lien rend `dashboard.html` accessible en un clic depuis n'importe quelle édition** — seul le code d'accès protège encore le contenu (décision assumée, pas une régression à corriger). `href="dashboard.html"` en toutes lettres (jamais un lien direct vers `en/` ou une archive) : recopier tel quel, comme le reste de `.masthead-right`.

   **Sujet révisé — automatique dès qu'une page de suivi existe, aucune saisie manuelle dans le cas courant.** Quand la routine de détection (`docs/routine-detection-prompt.md`) ajoute une nouvelle version à `suivi/{sujet}.html`, `scripts/seo/generate_archives_table.py` la détecte tout seul à la prochaine régénération hebdomadaire (via le `.origin-link` de la page de suivi, qui pointe vers l'archive d'origine) et met à jour la ligne correspondante d'`archives.html` avec **les chiffres de la dernière version** — « Notre scénario » (pourcentages) et « Impact France » (espérance recalculée avec ces pourcentages à jour) reflètent donc toujours la dernière évaluation, jamais les chiffres figés de l'édition d'origine. Un badge « Révisé » (lien vers la page de suivi) apparaît automatiquement, et le lien "Sujet révisé" du nav (`archives.html?tag=revise`) filtre sur ces lignes. **Rien à faire à la main dans ce cas.**

   Fallback : `<meta name="revised-on" content="{AAAA-MM-JJ}">` dans le `<head>` de l'archive, pour une révision mineure qui ne justifie pas une page de suivi dédiée (le badge apparaît, mais sans mise à jour des chiffres puisqu'il n'y a pas de nouvelle version à lire).

   **Bouton de bascule de langue (`.masthead-lang-btn`) — CSS dans le gabarit, mais bouton ajouté seulement par la routine EN, jamais par cette routine.** La classe CSS `.masthead-lang-btn` (juste après `.masthead-notif-btn` dans le `<style>`) fait partie du gabarit à recopier comme le reste du `<style>` — mais **ne jamais ajouter le lien `<a class="masthead-lang-btn">` lui-même dans `.masthead-right` depuis cette routine** : au moment où cette routine publie `index.html`, la traduction anglaise du jour n'existe pas encore (elle est produite après, voir étape 13 plus bas), donc aucune cible valide vers laquelle pointer. C'est la routine EN (`docs/routine-en-prompt.md`) qui ajoute ce bouton rétroactivement sur `index.html` et `archives/{AAAA-MM-JJ}.html`, une fois la traduction publiée — jamais cette routine-ci. Quand il est présent, il vient en tout premier enfant de `.masthead-right`, avant la cloche notifications.
   ...
   **Repositionnement de l'édition dans `.brand`, ajouté le 28 août [structure générale, même statut que `.masthead-right`].** `.brand` passe en deux lignes : la première (`.brand-row`) porte logo + wordmark ; la seconde porte `.edition`, hors de `.masthead-right` (qui ne contient que des boutons icône) :
   ...
   ```
   Repose sur toutes les pages du site (pas seulement `index.html`) — présent dans le `.masthead` partagé par `archives.html`, `glossaire.html`, `le-projet.html`, les pages `suivi/`, `hebdo/`, etc. (les pages sans édition n'ont simplement pas le second enfant `.edition`). **Ne jamais ajouter d'accroche texte (`.brand-tagline`) à droite du wordmark** — essayée puis retirée le même jour (trop discrète), remplacée par le bandeau d'accueil ci-dessous, bien plus visible.

   **Bandeau d'accueil premier passage (`.intro-banner`)** [structure générale — un premier visiteur qui atterrit directement sur l'édition du jour, cas le plus fréquent puisque `index.html` sert à la fois d'accueil et d'article, n'a aucun repère pour comprendre ce qu'est Scénario]. Juste après `</nav>`, avant la section suivante (plus de bande `.top-updates` intercalée entre les deux — voir plus haut) :
   ```html
   ...
   ```
   **Icône du logo (le tronc qui se sépare en 3 flèches favorable/stable/dégradé) plutôt que les libellés écrits en toutes lettres** — un essai avec 3 pastilles « Favorable/Stable/Dégradé » a été retiré le même jour (déjà dit une ligne plus bas par les vraies cartes de l'article). Ne jamais reformuler le texte du bandeau d'une édition à l'autre — fixe, comme la ligne `.masthead-right`.
...
**`<link rel="canonical">`** — placé juste après `<title>`. Suit la même règle de valeur qu'`og:url` : `<link rel="canonical" href="https://lesscenarios.fr/archives/{AAAA-MM-JJ}.html">` (pointe déjà vers l'archive permanente, **même sur `index.html`** — pas `https://lesscenarios.fr/` — puisque le contenu de l'accueil n'est qu'un miroir temporaire de cette archive-là, jamais une page distincte à faire indexer séparément), inchangé une fois copié dans l'archive (déjà auto-référent). Présent sur toutes les pages statiques du site sauf `bienvenue.html`/`confirmez-votre-email.html` (déjà `noindex`, un canonical n'y sert à rien).

**Longueur de `<meta name="description">`/`og:description`/`twitter:description`/JSON-LD `description` : viser ≤ 155-160 caractères.** Google tronque au-delà, souvent en plein milieu d'une phrase. La question posée complète (`.question-text` dans le corps de la page) **ne change jamais** et garde toute sa précision — seule la copie dans les balises meta/JSON-LD peut être raccourcie si elle dépasse cette longueur, en coupant à une frontière naturelle (comme la règle de troncature de la catégorie `chiffre` dans `docs/routine-pub-prompt.md`) plutôt qu'en reformulant le sens. Vérifier la longueur avant de valider l'étape 3bis ; si > 160 caractères, raccourcir uniquement les 4 champs meta/JSON-LD, jamais `.question-text`.
...
**Ne jamais mettre `author` en `Person` nommée** (avec `jobTitle`/`sameAs` LinkedIn personnel) ni de byline visible sous `.edition` — testé puis retiré (risque de s'identifier publiquement). Le nom reste mentionné uniquement dans `le-projet.html` (section « Qui fait Scénario »), sans lien externe.

**`sameAs` sur `author` (Organization), à la place** : pointe vers la **page LinkedIn de l'entreprise** (`https://www.linkedin.com/company/136694258/`, déjà publique et déjà liée depuis `le-projet.html`) — donne à Google un signal de légitimité (Organization vérifiable) sans exposer de personne nommée. Ne change jamais, identique FR/EN, pas de paramètre de tracking dans l'URL.
...
6. **Générer la vignette d'archive avant de construire l'entrée** :
```bash
...
7bis. **Mettre à jour `sitemap-news.xml`** (pour l'indexation Google Actualités). Contrairement à `sitemap.xml` (toutes les éditions, jamais purgé), **`sitemap-news.xml` ne garde que les articles des dernières 48h** — c'est la spécification officielle du protocole Google News sitemap, pas une convention du site. Chaque jour :
...
**Description structurée en 3 blocs étiquetés — Question / Faits / Scénarios** (même logique que le kicker `.section-label` « Les faits » sur le site, étape technique 3 plus haut, appliquée ici à l'email). Trois lignes labellisées, dans cet ordre, chacune précédée d'un double `<br>` :
...
Ne pas réécrire ces trois libellés d'une édition à l'autre : toujours exactement « La question posée : », « Les faits : », « Les 3 scénarios : ».
...
**Image du sujet — Pexels par défaut (essai avant l'image générée).** Une fois `archives/{AAAA-MM-JJ}.html` écrit, tenter une vraie photo libre de droits avant de retomber sur le visuel généré. **Pexels reste seul actif par défaut** (Pixabay testé en remplacement un temps : résultats décevants, stock clicheté, biais vers du contenu hors-sujet sur des requêtes génériques). Pixabay reste dormant (code fonctionnel, `--source pixabay`), **pas de bascule automatique vers Pixabay en cas d'échec Pexels** — voir ci-dessous le repli retenu à la place.
...
**Image en tête d'article — cover plein écran avec titre en incrustation.** Traitement « couverture de magazine » : photo plein bleed (100vw edge-to-edge), eyebrow/h1/date en incrustation ancrés en bas de l'image sur un dégradé sombre, légende de la photo en footnote tout en bas de page (voir plus bas). **Deux structures possibles selon qu'une image a été retenue ou non — ne jamais mélanger les deux :**
...
**Crédit photo en footnote, dans `<footer>`, jamais dans le corps de l'article** (une légende juste sous le cover plein écran arriverait trop tôt, avant même le premier mot de l'article). Seulement si une image a été retenue (cas 1), ajouter/mettre à jour **à l'intérieur de `.footer-bottom`, comme premier enfant, avant `.legal-links`** — jamais juste au-dessus de `.footer-bottom` avec son propre filet de séparation, ça donne l'impression de deux footnotes empilées au lieu d'une seule :
```html
<footer>
  <div class="wrap">
...
**L'envoi push est géré par Make.com, jamais par cette routine.** Le
scénario Make qui poste déjà sur Telegram/Twitter/Instagram/Facebook/
LinkedIn/Bluesky à partir de `feed.xml` inclut un module OneSignal
(`contents`/`url` tirés directement de l'item RSS). **Ne jamais
