...
2. Lire `index.html` actuel : gabarit de design exact à reproduire. Ne jamais changer le CSS ni la structure HTML générale — seulement le contenu texte et les valeurs. **La bande `.top-updates` juste sous la nav (boutons "Sujet révisé" / "Récap de la semaine", en `.btn-outline` avec icône SVG depuis le 19 août — plus d'emoji) fait partie de cette structure générale à reproduire telle quelle** — recopier les deux `<a class="btn-outline">` (icône SVG + texte) à l'identique. Le lien "Sujet révisé" pointe toujours vers `archives.html?tag=revise` (filtre générique, toujours à jour tout seul — ne jamais le changer). Le lien "Récap de la semaine" pointe vers le dernier `hebdo/{date}.html` publié : sa mise à jour est un geste séparé, pas une tâche de la routine quotidienne — recopier tel quel, ne jamais le recalculer ni le deviner.
   ...
   **Section `#nous-suivre` (« Nous suivre »), boutons passés en icônes seules + Telegram ajouté, le 21 août [structure générale]** — retour utilisateur : avec un 6e réseau (Telegram, absent jusque-là de cette section précise) le format bouton texte (« Sur LinkedIn »...) ne tenait plus sur une seule ligne, même sur desktop. Les 6 boutons `.follow-btn` sont maintenant des cercles icône seule (38px→36px, `aria-label`/`title` pour l'accessibilité, plus de texte visible), `#nous-suivre .share-row{ gap: 8px; }` pour que les 6 tiennent sur une ligne même à 320px de large (vérifié par capture Playwright). Ordre inchangé (X, Bluesky, LinkedIn, Facebook, Instagram), Telegram ajouté en dernier. Même composant `.follow-btn` que sur les autres pages du site (`glossaire.html`, `contact.html`...) — cohérence de nom conservée. CSS `.follow-btn` déjà dans le gabarit ; recopier le bloc `<style>` intégralement comme d'habitude.
