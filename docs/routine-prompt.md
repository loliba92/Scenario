...
<section class="scenarios">
  <div class="wrap">
    ...
    <div class="cards">
      <!-- les 3 cartes -->
```
**[CHANGÉ le 2 septembre, retour utilisateur : « ce qu'on évalue n'est pas clair, il doit être plus pédagogique, expliquer ce qu'on dit en favorable/stable/dégradé »].** Avant cette date, les 3 branches étaient enchaînées dans un seul `<p class="stakes-text">`, en questions (« Est-ce que... ? Est-ce que... ? Ou est-ce que... ? ») — le lien entre chaque sous-question et son scénario n'était porté que par l'ordre de lecture, invisible pour qui ne connaît pas déjà la convention. Désormais une liste, une ligne par branche, chacune ouverte par un `<span class="stakes-tag">` (le mot-repère Favorable/Stable/Dégradé, coloré comme `.kind-tag` sur les cartes) : le rapprochement entre une branche et sa carte se fait d'un coup d'œil, avant même d'atteindre les cartes. Le texte de chaque branche passe aussi en phrase déclarative simple (« L'Anses résiste et refuse... », pas « Est-ce que l'Anses résiste et refuse... ? ») — le mot-repère qui ouvre la ligne dit déjà qu'il s'agit d'une branche, la forme interrogative n'ajoutait rien. CSS (`.stakes-box`, `.stakes-label`, `.stakes-branches`, `.stakes-tag`) déjà dans le gabarit ; `.stakes-text` reste défini en CSS pour compatibilité avec les archives publiées avant le 2 septembre, mais ne doit plus être utilisé dans une nouvelle édition.

**Les 3 branches doivent rester dans l'ordre favorable/stable/dégradé, une par scénario, jamais une question ouverte vague.** Avant de publier, vérifier explicitement que chaque scénario (titre + `why`) répond sans ambiguïté à sa branche — si un scénario semble à côté, corriger le scénario ou la branche. **Ce texte n'est plus repris dans `feed.xml`** (retiré de la Description le 28 août, voir étape technique 8 plus bas — la référence à une réutilisation dans `feed.xml` a été retirée de ce paragraphe le 2 septembre, elle ne correspondait déjà plus à l'étape technique 8 depuis le 28 août).
...
**Ne pas confondre avec « Ce qu'on évalue » (`.stakes-branches`, ex-`.stakes-text`) : `<source>` est réservé à « L'essentiel » et rien d'autre.** Erreur commise une fois (11 août, corrigée) : « Ce qu'on évalue » copié dans `<source>` à la place de « L'essentiel » — vérifier que les deux textes ne sont jamais permutés. `<source>` alimente directement `{{4.source.title}}` sur les légendes Instagram/LinkedIn/Facebook (voir `docs/ARCHITECTURE.md`) : une confusion ici se propage silencieusement à tous ces canaux. **[CORRIGÉ le 2 septembre]** Ce paragraphe affirmait encore que « Ce qu'on évalue » sert de second paragraphe de la `<description>` — plus vrai depuis le 28 août, qui l'a retiré de la Description (voir étape technique 8 plus bas) sans que cette note-ci soit mise à jour à l'époque ; « Ce qu'on évalue » ne va plus nulle part dans `feed.xml` aujourd'hui, uniquement sur le site.
...
- **« Ce qu'on évalue » (`.stakes-branches`, format complet plus haut dans l'étape 3) : chaque branche est une phrase déclarative courte**, jamais une phrase à rallonge avec points-virgules et incises. Mauvais : « L'Anses refuse la dérogation, sous X, par Z ; ou l'accorde, avec Y ; ou l'étend — D — jusqu'à E. » Bon, une ligne par branche : « L'Anses refuse, ou limite fortement, la dérogation. » / « Elle l'accorde exactement comme prévu par la loi. » / « La dérogation s'étend au-delà du périmètre initial. » **[CHANGÉ le 2 septembre]** Avant cette date la consigne demandait 3 phrases interrogatives dans un unique `<p>` — remplacé par le format en liste étiquetée décrit plus haut, voir le retour utilisateur cité là-bas.
...
2. Lire `index.html` actuel : gabarit de design exact à reproduire. Ne jamais changer le CSS ni la structure HTML générale — seulement le contenu texte et les valeurs. **L'ancienne bande `.top-updates` (boutons "Sujet révisé" / "Récap de la semaine" en pleine largeur juste sous la nav) a été supprimée le 29 août — ne plus jamais la reproduire.** Retour utilisateur : ces deux liens ne méritaient pas leur propre bande, ils ont rejoint la ligne `.masthead-right` (voir juste en dessous).
   ...
   **[AJOUTÉ le 31 août 2026, ABANDONNÉ le 1er septembre 2026, retour utilisateur direct : « nous avions abandonné cette piste maintenant tu pointes directement vers archives.html ».** L'idée d'un menu déroulant "Archives" (bouton `.topnav-archives` + panneau `.topnav-archives-panel` à 7 liens) avait été documentée ici le 31 août, puis appliquée une première fois le 1er septembre — et immédiatement retirée le jour même sur retour utilisateur explicite : ce projet avait en fait déjà été abandonné avant, la note ci-dessus n'aurait jamais dû être appliquée. **Ne jamais reproduire ce bouton/panneau** — le lien `Archives` de `nav.topnav` reste un simple `<a href="archives.html">Archives</a>` (icône + texte, comme les autres liens de cette nav), exactement comme avant le 31 août. Si l'envie d'un point d'entrée vers les pages thématiques revient un jour, ne pas réutiliser ce patron sans un nouveau feu vert explicite — proposer plutôt une nouvelle piste.
