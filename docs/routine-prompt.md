...
```
Contenu attendu, toujours en 2-3 phrases, jamais plus : (1) décrire l'issue la plus probable avec son %, **en langage concret (chiffres, conséquence réelle) — jamais juste le mot "favorable"/"stable"/"dégradé" seul**, ajusté le 8 août après retour utilisateur : le lecteur ne connaît pas forcément ce que chaque label recouvre pour ce sujet précis, contrairement à quelqu'un qui vient de lire les 3 cartes en détail (mauvais exemple : « le scénario stable (45%) reste le plus probable » ; bon exemple : « le rebond se maintient sur un rythme soutenu sans s'accélérer (45%) »), (2) donner **un signal concret et vérifiable à surveiller** qui ferait basculer vers une autre issue — pas une généralité du type "il faudra voir comment ça évolue", un vrai indicateur observable (un événement daté, une publication de chiffres, une décision attendue). Ne jamais répéter mot pour mot ce qui est déjà dit dans les paragraphes `why` des cartes — c'est une synthèse qui relie les 3 scénarios entre eux, pas un résumé de l'un d'eux. Le style CSS (`.essentiel-box`, `.essentiel-label`, `.essentiel-text`) existe déjà dans le gabarit : ne pas le redéfinir. Le libellé « L'essentiel » a été choisi pour rester neutre (pas de connotation de verdict tranché comme « Conclusion », qui contredirait l'esprit du site) — sujet à ajustement par l'utilisateur, ne pas le changer de sa propre initiative si une session future y réfléchit.
