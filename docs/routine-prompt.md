...
**Dernier temps du contexte : un encart « Ce qu'on évalue », pas un simple paragraphe de conclusion vague.** Juste après `indicator-strip`, juste avant la section des scénarios (ordre inversé le 5 août, retour utilisateur : lire "Ce qu'on évalue" doit enchaîner directement sur les 3 scénarios, sans que les indicateurs chiffrés s'intercalent entre la question et sa réponse), ne pas terminer le contexte par une formule floue type « plusieurs trajectoires sont possibles » ou « la suite reste incertaine ». Utiliser à la place un encart dédié `<div class="stakes-box">` (même famille visuelle que `question-box` du haut de page, voir gabarit `index.html`) :
```html
