...
**Même texte repris dans `feed.xml`, ajouté le 8 août.** Une fois le bloc « L'essentiel » rédigé (voir ci-dessus), ajouter aussi un tag `<essentiel>{même texte, sans les balises HTML}</essentiel>` dans l'`<item>` du jour — juste après `<category>`, avant `<enclosure>`. Tag non standard (RSS 2.0 ne le prévoit pas), mis à disposition pour un usage futur côté Make.com si besoin, pas encore branché sur un module particulier — ne pas essayer de deviner un usage, juste le remplir avec le texte exact du bloc `essentiel-box` du jour, texte brut sans `<strong>` ni autre balisage.
