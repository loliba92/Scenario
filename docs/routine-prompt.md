...
**Le `{paragraphe d'intro}` ne doit jamais reformuler les mêmes faits/chiffres que l'accroche juste au-dessus** (piège constaté le 2 août sur l'édition Ceuta : accroche et intro répétaient toutes les deux « 60 000 personnes... 72 morts », quasi mot pour mot — redondant et sans intérêt à la lecture). L'accroche pose déjà le contexte et le chiffre choc : le `{paragraphe d'intro}` doit soit apporter un fait **différent** et complémentaire, soit se réduire à une simple phrase de transition vers les scénarios (« Trois scénarios se dessinent 👇 ») sans réintroduire les chiffres déjà cités. Avant de valider l'item, relire les deux paragraphes l'un à la suite de l'autre et vérifier qu'aucune information n'apparaît deux fois.
