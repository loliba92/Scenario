...
<figure class="article-image">
  <div class="article-image-photo-wrap">
    ...
    <p class="article-image-title" aria-hidden="true">{h1 du jour}</p>
  </div>
  <figcaption class="article-image-caption">Photo : {photographe} / <a href="{pexels_url}" target="_blank" rel="noopener noreferrer">Pexels ↗</a></figcaption>
</figure>
```
`{description factuelle courte}` : la même que celle déjà rédigée pour `og:image:alt` juste au-dessus, pas une nouvelle rédaction. `{h1 du jour}` reprend le `<h1>` de l'édition tel quel (le titre affiché dans l'image est décoratif — `aria-hidden="true"` — le vrai `<h1>` sémantique reste celui affiché plus haut sur la page, jamais un doublon lu par un lecteur d'écran). `{photographe}`/`{pexels_url}` viennent de la fiche de provenance (`assets/social/topic-images/{AAAA-MM-JJ}.json`). Les styles `.article-image*` sont déjà dans le `<style>` du gabarit depuis le 10 août — ne pas les redéfinir. **Si aucun fichier `-wide.jpg` n'existe** (pas de photo retenue ce jour-là, ou échec du recadrage large — voir `use_topic_image.py`), ne rien insérer : l'article reste sans image, comme pour toutes les éditions avant le 10 août — jamais bloquant pour la publication. Reporter le même bloc (ou son absence) sur `archives/{AAAA-MM-JJ}.html` au moment de créer la copie figée (étape technique 5) — cohérent avec le fait que l'archive doit être identique à l'édition publiée.
