...
<div class="follow-inline">
  <p class="follow-inline-text">Reste informé de la prochaine édition :</p>
...
**`context` : UN SEUL paragraphe de contexte affiché sous le titre, 1-2 phrases courtes (≤ 24 mots au total), pas deux légendes séparées.** Doit à la fois planter le décor et dire ce qui va déterminer l'issue. **Recycler en priorité ce qu'on a déjà sous la main plutôt que d'en réécrire un nouveau** (retour utilisateur, 24 août) : la meta `description`/`og:description` de l'édition (déjà rédigée à l'étape du gabarit, déjà vérifiée, déjà pensée pour être compréhensible hors contexte) convient presque toujours telle quelle — la recopier verbatim si elle tient dans le budget de mots, plutôt que de rédiger un texte concurrent qui raconterait la même chose autrement. Ne rédiger un texte dédié que si la meta description dépasse le budget ou ne se lit pas naturellement une fois isolée sur l'image. Jamais un copier-coller de la question posée ou de `.stakes-box` du site (bien trop longs pour tenir lisiblement). **Remplace depuis le 24 août les anciens champs séparés `hook` (accroche dorée) + `context` (ligne de contexte grise) sur deux lignes** — retour utilisateur explicite : deux légendes de couleurs différentes l'une sous l'autre « ça fait brouillon » ; un seul paragraphe, une seule couleur. Exemple pour l'édition Ukraine du 24 août (meta description recyclée telle quelle) : « Après quatre ans et demi de guerre, les pourparlers américains butent sur le Donbas : cessez-le-feu proche, guerre gelée, ou embrasement avant l'hiver ? » Committer le PNG (et la photo + fiche de provenance le cas échéant). Ajouter dans l'`<item>`, juste après `</category>` et avant `<description>` :
```xml
