...
```
**Le bloc doit être autonome, lisible seul sans avoir lu le reste de l'article** (ajusté le 8 août après retour utilisateur — un lecteur qui tombe directement sur ce bloc, par exemple via un partage ou un extrait, doit tout comprendre sans remonter au reste de la page). Contenu attendu, dans cet ordre, en 3-4 phrases (un peu plus que "2-3" maintenant que la problématique s'ajoute, mais toujours court — pas un second article) :
1. **Une phrase de contexte/problématique**, qui rappelle le fait clé qui a motivé l'édition et la question posée — pas un copier-coller de `question-text`, une reformulation aussi courte.
2. **L'issue la plus probable avec son %, en langage concret** (chiffres, conséquence réelle) — **jamais juste le mot "favorable"/"stable"/"dégradé" seul** : le lecteur ne connaît pas forcément ce que chaque label recouvre pour ce sujet précis (mauvais exemple : « le scénario stable (45%) reste le plus probable » ; bon exemple : « le rebond se maintient sur un rythme soutenu sans s'accélérer (45%) »).
3. **Un signal concret et vérifiable à surveiller** qui ferait basculer vers une autre issue — pas une généralité du type "il faudra voir comment ça évolue", un vrai indicateur observable (un événement daté, une publication de chiffres, une décision attendue).
