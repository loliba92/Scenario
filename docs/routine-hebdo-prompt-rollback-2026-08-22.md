# Rollback — prompt intégral de « Scénario — On refait le scénario de la semaine » (avant le 22 août 2026)

Ce fichier conserve tel quel le texte qui vivait en dur dans le trigger
`trig_01FwX1Q3xsLCMwAZt4WviUA6` avant le passage au mode pointeur du 22
août 2026 (voir `docs/routine-hebdo-prompt.md`, qui est désormais la
source de vérité vivante). En cas de problème avec la version allégée, le
recoller tel quel dans l'interface Claude Code Remote (champ prompt du
trigger) suffit à revenir en arrière.
