...
**Même jour, suite directe** : la rédaction a fini par réussir après enrichissement du brief (voir plus bas, § « Repli "meilleur essai" »), mais le run manuel de `post-edition.yml` n'a rien publié malgré tout — étape *« Committer et pousser »* **skippée**. Cause : `publish` (input `workflow_dispatch`) défaut à `false` en déclenchement manuel (mode bac à sable, tout reste sous `_prototype-out/`), et l'utilisateur ne l'avait pas mis à `true`.

**Défaut de `publish` inversé, retour utilisateur explicite** (« dans l'API il faut envoyer sur main toujours, c'est à moi de préciser pour envoyer sur un bac à sable »). En creusant, un vrai bug latent est ressorti : le trigger CCR (`docs/routine-prompt.md`, Étape 3bis) appelle `gh workflow run post-edition.yml --ref main` **sans jamais préciser `publish`** — tant que le défaut manuel restait `false`, une réactivation de ce trigger (aujourd'hui désactivé, voir plus haut) aurait fait tourner la routine quotidienne en bac à sable indéfiniment, sans jamais rien publier, sans erreur visible nulle part. Même classe de bug, et même principe de correction, que le défaut figé de l'input `brief` déjà corrigé le 15 septembre (voir le commentaire du step *« Calculer le brief... »* dans `post-edition.yml`) : jamais de valeur par défaut qui diverge silencieusement du comportement réel attendu. `publish` passé à `default: "true"` dans `.github/workflows/post-edition.yml` — un test manuel sans publication doit désormais préciser explicitement `publish=false`.
