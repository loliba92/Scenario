...
ARCHIVES_TABLE_CSS = """
  ...
  .hero.archives-hero .reads-total {
    position: relative;
    text-shadow: 0 2px 12px rgba(0, 0, 0, 0.5);
  }

  /* Total de lectures cumulées, toutes éditions confondues (FR + EN) — même
     source que la colonne "Lectures" du tableau (reads_script, plus bas),
     juste sommée plutôt que par édition. Masqué (attribut hidden) tant que
     le JSON n'a pas chargé : jamais de "0" trompeur avant que la vraie
     valeur arrive. */
  .hero.archives-hero .reads-total {
    margin: 4px 0 0;
  ...
  .hero.archives-hero .reads-total #reads-total-value {
    color: var(--gold);
    font-weight: 700;
  }
...
{reads_filter_html}
  ...
  (function(){
    ...
    fetch('assets/data/reads.json').then(function(res){
      ...
      // Total cumulé toutes éditions (FR + EN déjà fusionnées côté
      // update_reads_json.py) — même dict `reads`, juste sommé plutôt que
      // lu par édition. Affiché seulement si au moins une valeur connue,
      // jamais un "0" avant chargement (voir attribut hidden ci-dessus).
      var totalKeys = Object.keys(reads);
      if(totalKeys.length){
        var total = totalKeys.reduce(function(sum, key){ return sum + (reads[key] || 0); }, 0);
        var totalLine = document.getElementById('reads-total-line');
        var totalValue = document.getElementById('reads-total-value');
        if(totalLine && totalValue){
          totalValue.textContent = total.toLocaleString('fr-FR');
...
<section class="hero archives-hero">
  <div class="wrap">
    ...
    <p class="reads-total" id="reads-total-line" hidden><span id="reads-total-value">—</span> lectures cumulées depuis le lancement</p>
  </div>
</section>
